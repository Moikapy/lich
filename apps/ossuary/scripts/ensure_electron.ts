...
const electron_bin = path.join(dist_dir, "electron");

if (fs.existsSync(electron_bin)) {
  const existing = fs.existsSync(path_txt)
    ? fs.readFileSync(path_txt, "utf8").trim()
    : "";
  if (existing !== "electron") {
    fs.writeFileSync(path_txt, "electron");
  }
  process.exit(0);
}
...
if (!fs.existsSync(electron_bin)) {
  console.error("ensure_electron: electron binary missing after unzip");
  process.exit(1);
}
